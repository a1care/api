...
import { SubService } from "./src/modules/Services/subService.model";
import { ChildServiceModel as ChildService } from "./src/modules/Services/childService.model";
...
async function run() {
    ...
    // We search for "diagnostics" (lowercase) as seen in user's screenshot
    ...
    if (!vaccinations) {
        ...
        const vSub = await SubService.create({
        ...
        for (const item of vItems) {
            await ChildService.create({
                ...item,
                serviceId: vaccinations._id,
                subServiceId: vSub._id,
                selectionType: "SELECT",
                fulfillmentMode: "HOME_VISIT",
                imageUrl: vSub.imageUrl
            });
        }
    }
