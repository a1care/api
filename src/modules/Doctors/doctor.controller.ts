...
const getPartnerType = async (staff: any) => {
  if (!staff?.roleId) return null;

  const populatedRole = typeof staff.roleId === "object" && "code" in staff.roleId
    ? staff.roleId
    : await RoleModel.findById(staff.roleId).select("name code");

  const roleCode = populatedRole?.code ? String(populatedRole.code).toLowerCase() : null;
  const roleName = populatedRole?.name ? String(populatedRole.name).toLowerCase() : null;

  return roleCode || roleName || null;
};

const buildPartnerAuthResponse = async (staff: any, token: string) => {
  const partnerType = await getPartnerType(staff);
  ...
  return {
    ...
    partnerType,
    homeScreen: "PartnerHome" as const,
    profileCompleted: Boolean(staff?.isRegistered),
    user: {
      id: String(staff._id),
      ...
      isRegistered: Boolean(staff.isRegistered),
      status: staff.status ?? null,
      roleId: staff.roleId?._id ? String(staff.roleId._id) : staff.roleId ? String(staff.roleId) : null
    }
  };
};
...
export const verifyOtp = asyncHandler(async (req, res) => {
  ...
  if (otp && cleanMobile) {
    ...
    if (storedOtp && String(storedOtp) === String(otp)) {
      ...
      return res.status(200).json(
        new ApiResponse(200, "Verification successful", await buildPartnerAuthResponse(staff, token))
      );
    } else if (storedOtp) {
  ...
  try {
    ...
    return res.status(200).json(
      new ApiResponse(200, "Firebase Verification successful", await buildPartnerAuthResponse(staff, token))
    );
...
export const getStaffDetials = asyncHandler(async (req, res) => {
  ...
  const staffDetails = await doctorModel.findById(staffId).populate("roleId", "name code")
  console.log("DEBUG: Staff Details fetched:", { id: staffDetails?._id, profileImage: staffDetails?.profileImage });
  const partnerType = await getPartnerType(staffDetails);
  return res.status(200).json(
    new ApiResponse(200, "Staff details", {
      ...staffDetails?.toObject(),
      appType: "partner",
      userType: "partner",
      partnerType
    })
...
export const registerStaff = asyncHandler(async (req, res) => {
  ...
  const fields = [
    ...
    'email', 'vehicleNumber', 'vehicleType', 'businessName', 'gstNumber', 'fulfillmentMode'
  ];
